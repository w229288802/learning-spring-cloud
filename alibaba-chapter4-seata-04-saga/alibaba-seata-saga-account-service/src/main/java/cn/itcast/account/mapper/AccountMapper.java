...
public interface AccountMapper extends BaseMapper<Account> {
    ...
    @Update("update account_tbl set money = money + ${money} where user_id = #{userId}")
    int rollback(@Param("userId") String userId, @Param("money") int money);

    @Select("select 1 from account_tbl where user_id = #{userId} for update")
    Long selectForUpdate(@Param("userId") String userId);
}
